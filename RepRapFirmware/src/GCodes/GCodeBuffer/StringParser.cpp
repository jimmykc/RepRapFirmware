...
void StringParser::DecodeCommand() noexcept
{
	...
	if (cl == 'G' || cl == 'M' || cl == 'T'  || cl == 'S')
	{
	...
	else if (cl == 'X' || cl == 'Y' || cl == 'Z' || cl == 'U' || cl == 'V' ||
		cl == 'W' || cl == 'A' || cl == 'B' || cl == 'C' || cl == 'D'){
		commandLetter = cl;
		hasCommandNumber = false;
		commandNumber = -1;
		//modalCommandNumber = -1;
		...
		if (isdigit(gb.buffer[parameterStart]))
		{
			...
			{
				++parameterStart;
				modalCommandFraction = 0;
				int digi=0;
				do
				{
					if (isdigit(gb.buffer[parameterStart]))
					{
						modalCommandFraction = (10 * modalCommandFraction) + (gb.buffer[parameterStart] - '0');
						++parameterStart;
						++digi;
					}
				}while (isdigit(gb.buffer[parameterStart]));
				//integer to float
				modalCommandNumber = (float)modalCommandFraction;
				//float to fraction
				for(int i=0; i<digi; i++)	modalCommandNumber *= 0.1;
			}
		}
		//integer + fraction
		modalCommandNumber += commandNumber;
