...
bool BinaryParser::HasModalCommandNumber() const noexcept
{
	return (bufferLength != 0 && (header->flags & CodeFlags::HasMajorCommandNumber) != 0);
}
...
float BinaryParser::GetModalCommandNumber() const noexcept
{
	return HasModalCommandNumber() ? header->majorCode : -1;
}
