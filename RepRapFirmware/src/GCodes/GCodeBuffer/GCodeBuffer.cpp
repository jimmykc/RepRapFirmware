...
constexpr ObjectModelTableEntry GCodeBuffer::objectModelTable[] =
{
	...
	{ "feedRate",			OBJECT_MODEL_FUNC(self->machineState->feedRate, 1),							ObjectModelEntryFlags::none },
	...
	{ "state",				OBJECT_MODEL_FUNC(self->GetStateText()),									ObjectModelEntryFlags::none },
...
bool GCodeBuffer::HasModalCommandNumber() const noexcept
{
	return PARSER_OPERATION(HasModalCommandNumber());
}
...
float GCodeBuffer::GetModalCommandNumber() const noexcept
{
	return PARSER_OPERATION(GetModalCommandNumber());
}
