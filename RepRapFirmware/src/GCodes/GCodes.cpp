...
# endif
	...
	for (size_t axis = 0; axis < numVisibleAxes; axis++)
	{
		if (gb.Seen(axisLetters[axis]))
		{
			...
			//diameter mode
			if(IsDiameterMode && GetMachineType()==MachineType::LatheMode && axis == X_index){
				moveArg /=2;
			}

			//Rotational axis
			//if (platform.IsAxisRotational(axis) && gb.LatestMachineState().usingInches)
			if (gb.LatestMachineState().usingInches){
				//A, B, C, D, V ,rotational
				if (axis == A_index || axis == B_index || axis == C_index || axis == D_index || axis == U_index || axis == V_index)
				{
					//InchToMm ConvertDistance
					//debugPrintf("arg: %f\n",moveArg);
					moveArg = gb.InverseConvertDistance(moveArg);
					//debugPrintf("arg: %f\n",moveArg);
					//debugPrintf("axis: %d,%d\n",axis,gb.LatestMachineState().usingInches);
				}
			}
			...
			{
				// if(axis == X_index){
				// 	debugPrintf("eelse if (rp != nullptr)\n");
				// }
...
bool GCodes::DoArcMove(GCodeBuffer& gb, bool clockwise, const char *& err)
{
	...
	unsigned int axis0;
	unsigned int axis1;
	//Use axisI, axisK to map I,J and K.
	unsigned int axisI;
	unsigned int axisJ;

	//Milling :  XY, ZX and YZ / IJ, KI and JK
	if(machineType == MachineType::MillingMode){
		axis0 = (unsigned int[]){ X_index, Z_index, Y_index }[selectedPlane];
		axis1 = (axis0 + 1) % 3;
		axisI = (axis0 + 1) % 3;
		axisJ = (axisI + 1) % 3;
	}
	...
	//5 Axis :  XY, ZX and YZ / IJ, KI and JK
	else if(machineType == MachineType::FiveAxisBCMode){
		axis0 = (unsigned int[]){ X_index, Z_index, Y_index }[selectedPlane];
		axis1 = (axis0 + 2) % 3;
		if(axis0 == 0) axisI = 1;
		else if (axis0 == 1) axisI = 0;
		else if (axis0 == 2) axisI = 2;
		axisJ = (axisI + 1) % 3;
	}
	else{
		err = "Error mode";
...
//	int convertdatax = static_cast<int>(X_index);
//	int convertdatay = static_cast<int>(Y_index);
//	debugPrintf("X_index %d Y_index %d \n", convertdatax , convertdatay);
//	debugPrintf("axis0= %d axis1= %d \n", axis0 , axis1);
//	debugPrintf("initialUserX %0.3f initialUserY %0.3f \n", (double)moveBuffer.initialUserC0 ,(double)moveBuffer.initialUserC1 );
...
//	debugPrintf("newAxis0Pos %0.3f %0.3f %0.3f %0.3f \n",
//	(double)newAxis0Pos ,(double)moveBuffer.initialUserC0,(double)GetCurrentToolOffset(axis0),(double)GetWorkplaceOffset(axis0));
	...
	if (gb.Seen(axisLetters[axis1]))
	{
		...
		//diameter mode
		if(IsDiameterMode && GetMachineType()==MachineType::LatheMode ){
			newAxis1Pos /=2;
		}
		if (gb.LatestMachineState().axesRelative)
		{
...
//	debugPrintf("newAxis1Pos %0.3f %0.3f %0.3f %0.3f \n",
//	(double)newAxis1Pos ,(double)moveBuffer.initialUserC1,(double)GetCurrentToolOffset(axis1),(double)GetWorkplaceOffset(axis1));
...
//		debugPrintf("rParam= %0.3f %0.3f %0.3f\n",(double)rParam ,(double)iParam,(double)jParam);
	}
	else
	{
		//axisI
		if (gb.Seen((char)('I' + axisI)))
...
//		debugPrintf("iParam= %0.3f \n",(double)iParam);
//		debugPrintf("jParam= %0.3f \n",(double)jParam);
	}
...
//	debugPrintf("userArcCentreAxis0 %0.3f\n",  (double)userArcCentreAxis0 );
//	debugPrintf("userArcCentreAxis1 %0.3f\n",  (double)userArcCentreAxis1 );
...
//	moveBuffer.arcRadius, moveBuffer.arcCurrentAngle * RadiansToDegrees, moveBuffer.arcAngleIncrement * RadiansToDegrees, moveBuffer.segmentsLeft);
...
//				int convertdatay = static_cast<int>(drive);
//				debugPrintf("doingArcMove Y dirve %d %0.3f \n", convertdatay , (double)moveBuffer.initialCoords[drive] );
			}
			//&& Tool::GetXAxes(moveBuffer.tool).IsBitSet(drive)
			//else if (doingArcMove && drive == X_index )
...
//				int convertdatay = static_cast<int>(drive);
//				debugPrintf("doingArcMove X dirve %d %0.3f \n", convertdatay , (double)moveBuffer.initialCoords[drive] );
			}
			else
			{
...
//				int convertdatay = static_cast<int>(drive);
//				bool b1 = moveBuffer.doingArcMove;
//				bool b2 = Tool::GetYAxes(moveBuffer.tool).IsBitSet(drive);
//				bool b3 = Tool::GetXAxes(moveBuffer.tool).IsBitSet(drive);
//				debugPrintf("else dirve %d \n", convertdatay );
//				debugPrintf(b1 ? "true " : "false \n");
//				debugPrintf(b2 ? "true " : "false \n");
//				debugPrintf(b3 ? "true " : "false \n");
...
GCodeResult GCodes::ManageTool(GCodeBuffer& gb, const StringRef& reply)
{
	...
	//tool length
	float _toolLength = 0.0;
	if (gb.Seen('Z'))
	{
		_toolLength = gb.GetFValue();
	}
	...
	if (seen)
	{
		...
		else
		{
			...
			tool->SetRetractLength(_toolLength);
			//debugPrintf("tool RetractLength = %f", (double)tool->GetRetractLength() );
			reprap.AddTool(tool);
		}
	}
	else
...
const char* GCodes::GetMachineModeString() const noexcept
{
	...
	case MachineType::laser:
		return "Laser";
	case MachineType::LatheMode:
		if(IsDiameterMode) return "Lathe Mode - Diameter";
		else return "Lathe Mode - Radius";
	case MachineType::MillingMode:
		return "Milling Mode";
	case MachineType::DefaultMode:
		return "Default Mode";
	case MachineType::HMCMode:
		return "HMC Mode";
	case MachineType::FiveAxisBCMode:
		return "5 Axis BC Mode";
	case MachineType::LatheMode_Tailstock:
		return "Lathe Mode - Tailstock";
	case MachineType::MillingMode_Tailstock:
		return "Milling Mode - Tailstock";
	case MachineType::HMCMode_Tailstock:
		return "HMC Mode - Tailstock";
	case MachineType::FiveAxisBCMode_Tailstock:
		return "5 Axis BC Mode - Tailstock";
	default:
		return "Unknown";
	}
}
