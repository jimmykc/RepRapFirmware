...
void RotaryX(float xyzPos[],float _angle){
	//Y axis
	float _y_value = xyzPos[1] * cos(DegreesToRadians * _angle) - xyzPos[2] * sin(DegreesToRadians * _angle);
	//Z axis
	float _z_value = xyzPos[1] * sin(DegreesToRadians * _angle) + xyzPos[2] * cos(DegreesToRadians * _angle);
	//
	xyzPos[1] = _y_value;
	xyzPos[2] = _z_value;
}
// void RotaryY(float xyzPos[],float _angle,float _radius){
// 	//X axis
// 	float _x_value = xyzPos[0] + ( _radius * sin(DegreesToRadians * _angle) );
// 	//Z axis
// 	float _z_value = xyzPos[2] + ( _radius * cos(DegreesToRadians * _angle)) - _radius ;
// 	//
// 	xyzPos[0] = _x_value;
// 	xyzPos[2] = _z_value;
// }
void RotaryY(float xyzPos[],float _angle){
	//X axis
	float _x_value = -xyzPos[0] * cos(DegreesToRadians * _angle) + xyzPos[2] * sin(DegreesToRadians * _angle);
	//Z axis
	float _z_value = -xyzPos[0] * -sin(DegreesToRadians * _angle) + xyzPos[2] * cos(DegreesToRadians * _angle);
	//
	xyzPos[0] = _x_value;
	xyzPos[2] = _z_value;
}
void RotaryZ(float xyzPos[],float _angle,float _radius){
	//X axis
	float _x_value = xyzPos[0] + ( _radius * sin(DegreesToRadians * _angle) );
	//Y axis
	float _y_value = xyzPos[1] + ( _radius * cos(DegreesToRadians * _angle)) - _radius ;
	xyzPos[0] = _x_value;
	xyzPos[1] = _y_value;
}
											...
											int32_t motorPos[], bool isCoordinated) const noexcept
{
	...
	float  _radius = 0.0;
	size_t i_Cindex = 0, i_Zindex = 0, i_Yindex = 0, i_Xindex = 0, i_Bindex = 0, i_Uindex = 0, i_Windex = 0;
	Tool * const currentTool = reprap.GetCurrentTool();
	// for (size_t motor = 0; motor < numTotalAxes; ++motor){
	// 	//x axis
	// 	if(reprap.GetGCodes().GetAxisLetters()[motor] == 'X'){
	// 		i_Xindex = motor;
	// 		break;
	// 	}
	// }
	// debugPrintf("X=%f\n", (double)machinePos[i_Xindex]);
	if(reprap.GetGCodes().G235_active){
		for (size_t motor = 0; motor < numTotalAxes; ++motor){
			//angle B
			if(reprap.GetGCodes().GetAxisLetters()[motor] == 'B'){
				i_Bindex = motor;
			}
			//angle C
			if(reprap.GetGCodes().GetAxisLetters()[motor] == 'C'){
				i_Cindex = motor;
			}
			//x axis
			if(reprap.GetGCodes().GetAxisLetters()[motor] == 'X'){
			...
			//y axis
			if(reprap.GetGCodes().GetAxisLetters()[motor] == 'Y'){
				i_Yindex = motor;
			}
			//z axis
			if(reprap.GetGCodes().GetAxisLetters()[motor] == 'Z'){
				i_Zindex = motor;
			}
			//U axis
			if(reprap.GetGCodes().GetAxisLetters()[motor] == 'U'){
				i_Uindex = motor;
			}
			//W axis
			if(reprap.GetGCodes().GetAxisLetters()[motor] == 'W'){
				i_Windex = motor;
			}
		}
		if (currentTool == nullptr)
		{
			_radius = reprap.GetGCodes().G235_gap;
			//debugPrintf("Tool gone! %f \n",99 );
		}else{
			_radius = currentTool->GetRetractLength() - reprap.GetGCodes().G235_gap;
			//debugPrintf("Total=%f , gap=%f , length=%f \n", (double)_radius, (double)reprap.GetGCodes().G235_gap, (double)currentTool->GetRetractLength());
		}
		//debugPrintf("X=%f , Y=%f , Z=%f \n", (double)machinePos[i_Xindex], (double)machinePos[i_Yindex], (double)machinePos[i_Zindex]);
	}
	...
	for (size_t motor = 0; motor < numTotalAxes; ++motor)
	{
		...
		if (axis < axisLimit)
		{
			...
			if(motor == 0){
				if(reprap.GetGCodes().RestoreBeforeDistance[0] == -9999.0 ){
					reprap.GetGCodes().RestoreBeforeDistance[0] = movement;
					//debugPrintf("BacklashDistance FirstTime! %f \n",(double)reprap.GetGCodes().RestoreBeforeDistance[0] );
				}else{

					if(reprap.GetGCodes().RestoreBeforeDistance[0] > movement)
					{
						// int convertdatay = static_cast<int>(motor);
						// debugPrintf("X_index %d \n", convertdatay );
						// debugPrintf("Tool gone! %f \n",(double)reprap.GetGCodes().RestoreBeforeDistance[0] );
						reprap.GetGCodes().RestoreBeforeDistance[0] = movement;
						movement += reprap.GetGCodes().BacklashDistance[0];
					}else{
						reprap.GetGCodes().RestoreBeforeDistance[0] = movement;
					}
				}
			}else if(motor == 1){
				if(reprap.GetGCodes().RestoreBeforeDistance[1] == -9999.0 ){
					reprap.GetGCodes().RestoreBeforeDistance[1] = movement;
				}else{
					if(reprap.GetGCodes().RestoreBeforeDistance[1] > movement)
					{
						reprap.GetGCodes().RestoreBeforeDistance[1] = movement;
						movement += reprap.GetGCodes().BacklashDistance[1];
					}else{
						reprap.GetGCodes().RestoreBeforeDistance[1] = movement;
					}
				}
			}else if(motor == 2){
				if(reprap.GetGCodes().RestoreBeforeDistance[2] == -9999.0 ){
					reprap.GetGCodes().RestoreBeforeDistance[2] = movement;
				}else{
					if(reprap.GetGCodes().RestoreBeforeDistance[2] > movement)
					{
						reprap.GetGCodes().RestoreBeforeDistance[2] = movement;
						movement += reprap.GetGCodes().BacklashDistance[2];
					}else{
						reprap.GetGCodes().RestoreBeforeDistance[2] = movement;
					}
				}
			}else if(motor == 3){
				if(reprap.GetGCodes().RestoreBeforeDistance[3] == -9999.0 ){
					reprap.GetGCodes().RestoreBeforeDistance[3] = movement;
				}else{
					if(reprap.GetGCodes().RestoreBeforeDistance[3] > movement)
					{
						reprap.GetGCodes().RestoreBeforeDistance[3] = movement;
						movement += reprap.GetGCodes().BacklashDistance[3];
					}else{
						reprap.GetGCodes().RestoreBeforeDistance[3] = movement;
					}
				}
			}else if(motor == 4){
				if(reprap.GetGCodes().RestoreBeforeDistance[4] == -9999.0 ){
					reprap.GetGCodes().RestoreBeforeDistance[4] = movement;
				}else{
					if(reprap.GetGCodes().RestoreBeforeDistance[4] > movement)
					{
						reprap.GetGCodes().RestoreBeforeDistance[4] = movement;
						movement += reprap.GetGCodes().BacklashDistance[4];
					}else{
						reprap.GetGCodes().RestoreBeforeDistance[4] = movement;
					}
				}
				...
				if(reprap.GetGCodes().RestoreBeforeDistance[motor] == -9999.0 ){
					reprap.GetGCodes().RestoreBeforeDistance[motor] = movement;
				}else{
					if(reprap.GetGCodes().RestoreBeforeDistance[motor] > movement)
					{
						reprap.GetGCodes().RestoreBeforeDistance[motor] = movement;
						movement += reprap.GetGCodes().BacklashDistance[motor];
					}else{
						reprap.GetGCodes().RestoreBeforeDistance[motor] = movement;
					}
				}
	...
	// if(reprap.GetGCodes().G234_active){
	// 	RotaryX(xyzPos,machinePos[i_Cindex]);
	// 	debugPrintf("X=%f , Y=%f , Z=%f \n", (double)xyzPos[0], (double)xyzPos[1], (double)xyzPos[2]);
	// }
	if((int)reprap.GetGCodes().GetMachineType() == 6){
		for (size_t motor = 0; motor < numTotalAxes; ++motor){
			//z axis
			if(reprap.GetGCodes().GetAxisLetters()[motor] == 'Z'){
				i_Zindex = motor;
		...
		float movement = 0.0;
		//debugPrintf("befor Z=%0.3f \n", (double)machinePos[i_Zindex]);
		movement = machinePos[i_Zindex] + reprap.GetGCodes().G235_gap;
		//debugPrintf("after Z=%0.3f \n", (double)movement);
		motorPos[i_Zindex] = lrintf(movement * stepsPerMm[i_Zindex]);
	}
	if(reprap.GetGCodes().G235_active){
		float xyzPos[] = {machinePos[i_Xindex],machinePos[i_Yindex],machinePos[i_Zindex]};
		//debugPrintf("X=%f , Y=%f , Z=%f , gap=%f \n", (double)xyzPos[0], (double)xyzPos[1], (double)xyzPos[2] , (double)_radius);
		RotaryY(xyzPos , machinePos[i_Bindex] );
		//debugPrintf("X=%f , Y=%f , Z=%f , gap=%f \n", (double)xyzPos[0], (double)xyzPos[1], (double)xyzPos[2] , (double)_radius);
		//RotaryZ(xyzPos , machinePos[i_Cindex] , _radius );
		//debugPrintf("X=%f , Y=%f , Z=%f , gap=%f \n", (double)xyzPos[0], (double)xyzPos[1], (double)xyzPos[2] , (double)_radius);
		motorPos[i_Xindex] = lrintf(xyzPos[0] * stepsPerMm[i_Xindex]);
		motorPos[i_Yindex] = lrintf(xyzPos[1] * stepsPerMm[i_Yindex]);
		motorPos[i_Zindex] = lrintf(xyzPos[2] * stepsPerMm[i_Zindex]);
	}
	//diameter mode
	// if(reprap.GetGCodes().IsDiameterMode && reprap.GetGCodes().GetMachineType()==MachineType::LatheMode ){
	// 	motorPos[reprap.GetGCodes().X_index] /=2;
	// }
	return true;
}
