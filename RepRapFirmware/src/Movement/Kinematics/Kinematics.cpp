...
AxesBitmap Kinematics::GetHomingFileName(AxesBitmap toBeHomed, AxesBitmap alreadyHomed, size_t numVisibleAxes, const StringRef& filename) const noexcept
{
	...
	for (size_t axis = 0; axis < numVisibleAxes; ++axis)
	{
		if (toBeHomed.IsBitSet(axis) && (axis != Z_AXIS || !homeZLast || (alreadyHomed & homeFirst) == homeFirst))
		{
			...
			if(reprap.GetGCodes().GetMachineType() == MachineType::HMCMode){
				if(reprap.GetGCodes().GetAxisLetters()[axis] == 'Y'){
					filename.cat("_MillV");
				}else if(reprap.GetGCodes().GetAxisLetters()[axis] == 'X'){
					filename.cat("_MillT");
				}else if(reprap.GetGCodes().GetAxisLetters()[axis] == 'W'){
					filename.cat("_TailT");
				}else if(reprap.GetGCodes().GetAxisLetters()[axis] == 'Z'){
					filename.cat("_HeadT");
				}else if(reprap.GetGCodes().GetAxisLetters()[axis] == 'U'){
					filename.cat("_TurretT");
				}else{
					filename.cat(tolower(reprap.GetGCodes().GetAxisLetters()[axis]));
				}
			}
			else if(reprap.GetGCodes().GetMachineType() == MachineType::FiveAxisBCMode){
				if(reprap.GetGCodes().GetAxisLetters()[axis] == 'X'){
					filename.cat("_MillV");
				}else if(reprap.GetGCodes().GetAxisLetters()[axis] == 'Y'){
					filename.cat("_MillT");
				}else if(reprap.GetGCodes().GetAxisLetters()[axis] == 'W'){
					filename.cat("_TailT");
				}else if(reprap.GetGCodes().GetAxisLetters()[axis] == 'Z'){
			...
			else if(reprap.GetGCodes().GetMachineType() == MachineType::LatheMode_Tailstock){
				if(reprap.GetGCodes().GetAxisLetters()[axis] == 'Y'){
					filename.cat("_MillV");
				}else if(reprap.GetGCodes().GetAxisLetters()[axis] == 'U'){
					filename.cat("_MillT");
			...
			else if(reprap.GetGCodes().GetMachineType() == MachineType::MillingMode_Tailstock){
				if(reprap.GetGCodes().GetAxisLetters()[axis] == 'Z'){
					filename.cat("_MillV");
				}else if(reprap.GetGCodes().GetAxisLetters()[axis] == 'Y'){
					filename.cat("_MillT");
				}else if(reprap.GetGCodes().GetAxisLetters()[axis] == 'X'){
			...
			else if(reprap.GetGCodes().GetMachineType() == MachineType::HMCMode_Tailstock){
				if(reprap.GetGCodes().GetAxisLetters()[axis] == 'Y'){
					filename.cat("_MillV");
				}else if(reprap.GetGCodes().GetAxisLetters()[axis] == 'X'){
					filename.cat("_MillT");
			...
			else if(reprap.GetGCodes().GetMachineType() == MachineType::FiveAxisBCMode_Tailstock){
				if(reprap.GetGCodes().GetAxisLetters()[axis] == 'X'){
					filename.cat("_MillV");
				}else if(reprap.GetGCodes().GetAxisLetters()[axis] == 'Y'){
					filename.cat("_MillT");
				...
				filename.cat(tolower(reprap.GetGCodes().GetAxisLetters()[axis]));
				if(reprap.GetGCodes().GetMachineType() == MachineType::LatheMode){
					if(reprap.GetGCodes().GetAxisLetters()[axis] == 'X'
					|| reprap.GetGCodes().GetAxisLetters()[axis] == 'U'
					|| reprap.GetGCodes().GetAxisLetters()[axis] == 'Z'){
						filename.cat("_Lathe");
					}
				}else if(reprap.GetGCodes().GetMachineType() == MachineType::MillingMode){
					if(reprap.GetGCodes().GetAxisLetters()[axis] == 'X'
					|| reprap.GetGCodes().GetAxisLetters()[axis] == 'Y'
					|| reprap.GetGCodes().GetAxisLetters()[axis] == 'Z'){
						filename.cat("_Milling");
					}
				}
			}
